...
git_access_and_knowledge <- rbind(tables$git_access, tables$git_knowledge) %>%
  dplyr::mutate(name = c(rep("Access", 3), rep("Knowledge", 3))) %>%
  dplyr::select(name, value, n)

plot <- CARS::plot_stacked(git_access_and_knowledge, n = samples$all, xlab = "", colour_scale = "3scale", font_size = 14, orientation = "v")
table <- CARS::df_to_table(git_access_and_knowledge, n = samples$all, column_headers = c("Knowledge / Access", "Response", "Percent"))

CARS::wrap_outputs("git-access-and-knowledge", plot, table)
