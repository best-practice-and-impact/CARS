...
Given a list of programming tools, we asked respondents if the tool was available to use for their work. 
...
Given the same list of programming tools, respondents were asked if they knew how to program with the tool to a level suitable for their work, answering "Yes", "No" or "Not required for my work". 
...
Differences in preferred languages may lead to silos between analytical professions. Here we show the tool capability of members of the different analytical professions.

Please note that respondents might be members of more than one profession.

```{R}
colnames(tables$languages_by_prof)[1] <- "Profession"

tables$languages_by_prof %>% CARS::df_to_table(crosstab = T, proportion_col = 3, sample = samples$profs[samples$profs > 0])
```
