...
In 2026, we also asked respondents whether they would like to use more coding in their role, and `r round(sum(data$coding_freq_pref == "Yes") / nrow(data) * 100, 1)`% said that they would.
...
plot <- CARS::plot_freqs(data = tables$git, 
                         ...
                         orientation = "h") |> 
  CARS::set_axis_range(0, 1, axis = "x")
...
coding_improve_barriers <- dplyr::arrange(tables$coding_improve_barriers, value, desc(n))
...
table <- CARS::df_to_table(coding_improve_barriers,  
                           column_headers = c("Barrier", "Yes", "No"), 
                           crosstab = TRUE)
...
coding_learn_barriers <- dplyr::arrange(tables$coding_learn_barriers, value, desc(n))
...
ai_use_data <- dplyr::arrange(ai_use_data, value, n)

plot <- CARS::plot_stacked(data = ai_use_data,
                           colour_scale = "scale",
                   font_size = 14)

table <- CARS::df_to_table(data = ai_use_data,
...
cloud <- dplyr::arrange(tables$cloud, value, desc(n))
...
table <- CARS::df_to_table(cloud,  
                           column_headers = c("Data Platform", "Yes", "No"), 
                           crosstab = TRUE)
...
rap_barriers <- dplyr::arrange(tables$rap_barriers, value, desc(n))
...
Respondents were asked whether they follow any analysis or coding quality standards. The majority indicated that they do, using either internal standards, external guidance, or a combination of both.
