...
data <- data[!is.na(data$department) & data$department == "Northern Ireland Statistics and Research Agency", ]

tables <- CARS::summarise_all(data, config, all_tables = FALSE)

tmp_dir <- file.path(
  tempdir(),
  janitor::make_clean_names("Northern Ireland Statistics and Research Agency")
)
...
```{r}
CARS::create_table_download_zip(
  tmp_dir = tmp_dir,
  zip_file = paste0(
    janitor::make_clean_names("Northern Ireland Statistics and Research Agency"),
    ".zip"
  ),
  label = paste("Download", "Northern Ireland Statistics and Research Agency", "tables")
)
