...
data <- data[!is.na(data$department) & data$department == "Office for National Statistics", ]

tables <- CARS::summarise_all(data, config, all_tables = FALSE)

tmp_dir <- file.path(
  tempdir(),
  janitor::make_clean_names("Office for National Statistics")
)
...
CARS::create_table_download_zip(
  ...
  label = paste("Download", "Office for National Statistics", "tables")
)
