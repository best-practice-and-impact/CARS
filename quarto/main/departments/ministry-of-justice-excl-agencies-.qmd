...
data <- data[!is.na(data$department) & data$department == "Ministry of Justice (excl. agencies)", ]

tables <- CARS::summarise_all(data, config, all_tables = FALSE)

tmp_dir <- file.path(
  tempdir(),
  janitor::make_clean_names("Ministry of Justice (excl. agencies)")
)
...
CARS::create_table_download_zip(
  ...
  label = paste("Download", "Ministry of Justice (excl. agencies)", "tables")
)
