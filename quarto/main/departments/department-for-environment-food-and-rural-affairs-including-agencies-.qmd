...
data <- data[data$defra, ]

tables <- CARS::summarise_all(data, config, all_tables = FALSE)

tmp_dir <- file.path(
  tempdir(),
  janitor::make_clean_names("Department for Environment, Food and Rural Affairs (including agencies)")
)
...
CARS::create_table_download_zip(
  ...
  label = paste("Download", "Department for Environment, Food and Rural Affairs (including agencies)", "tables")
)
