...
data <- data[!is.na(data$department) & data$department == "Scottish Government (excl. agencies)", ]

tables <- CARS::summarise_all(data, config, all_tables = FALSE)

tmp_dir <- file.path(
  tempdir(),
  janitor::make_clean_names("Scottish Government (excl. agencies)")
)
...
CARS::create_table_download_zip(
  ...
  label = paste("Download", "Scottish Government (excl. agencies)", "tables")
)
