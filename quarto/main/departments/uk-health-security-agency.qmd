...
data <- data[!is.na(data$department) & data$department == "UK Health Security Agency", ]

tables <- CARS::summarise_all(data, config, all_tables = FALSE)

tmp_dir <- file.path(
  tempdir(),
  janitor::make_clean_names("UK Health Security Agency")
)
...
CARS::create_table_download_zip(
  ...
  label = paste("Download", "UK Health Security Agency", "tables")
)
