...
The coding in analysis and research survey (CARS) data collection takes place for approximately one month, every Autumn. Launch dates vary slightly by year to maximise response rate, for example by avoiding clashes with other internal surveys. In 2022, data collection took place from 3 October to 11 November.

Recruitment was done through a variety of channels, mailing lists, networks and newsletters. For the past three years, the most common source of data was through RAP champions, who promote the survey in their organisations. We rely on various champions, heads of professions (HoPs) and deputy directors of analysis (DDans) to promote the survey in their departments. This means the response rate and any selection bias will vary across organisations. 

Our promotional materials make it clear that we are interested in responses from all analysts, whether or not they code. However, it may be the case that the survey attracts a disproportionate number of respondents who have an interest in coding and RAP. We advise against making strong inferences about differences between professions and departments or attempting to estimate real frequencies from the data. 
...
Every year, we ask respondents how often they code to achieve work objectives. While our communication strategy has changed over time, particularly to encourage more non-coders to respond, the findings remain similar, with a gradual increase in the number of coders over time.
Although we seek responses from all analysts the data likely over-represents people with current or prior coding experience.
...
Across all waves, over 80% of respondents from the civil service are at HEO to grade 7 grades. While this will be representative of the grade distribution of analysts in some government organisations, it may not be the case for all organisations.
...
Below is a breakdowns for civil service professions. These cover the [analysis function professions](https://analysisfunction.civilservice.gov.uk/about-us/frequently-asked-questions/) and do not apply outside of the civil service. The exception to this are data scientists who do not have an official government profession, but are included separately here to avoid skewing the data for other professions. Please note that respondents can have multiple profession memberships. Profession data is difficult to compare across waves as these questions have changed in line with changes to the analysis function.

The CARS sample has high representation from statisticians in particular compared with other professions. This again may be representative of some organisations but not all. 
