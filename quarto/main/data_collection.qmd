...
The Coding in Analysis and Research Survey (CARS) data collection takes place for approximately one month, every autumn/winter. The survey is self-selecting and participation is voluntary. Launch dates vary slightly by year to maximise response rate, for example by avoiding clashes with other internal surveys. In 2026, data collection took place from 26 Janurary to 13 March.
...
Lastly, while the survey is open to all public sector analysts, the vast majority of responses come from the UK and devolved Civil Service (`r round(sum(data$workplace == "Civil service, including devolved administrations") / nrow(data) * 100, 1)`% in 2026). As such, follow-up questions on grade and profession applied only to civil servants.

## Where our data comes from

Link tracking allows us to see where responses are coming from. Links promoted by RAP champions were the most commonly used for the past three waves, and accounted for over half of responses in 2026.
...
CARS::df_to_table(tracking_link_freqs[c(2,1,5)], 
                  column_headers = c("Tracking link", "2022", "2023", "2024", "2026"), 
                  crosstab = T,
                  sample = FALSE)
...
Every year, we ask respondents how often they code to achieve work objectives. In 2026, this question was only asked of respondents with current or prior coding experience. Although we seek responses from all analysts the data probably over-represents people with coding experience. In 2026, `r round(sum(data$code_freq == "Never" | is.na(data$code_freq)) / nrow(data) * 100, 1)`% of respondents never used code at work or had no coding experience.
...
The CARS sample has high representation from statisticians compared with other professions. This again may be representative of some organisations but not all, and is not representative of the [Civil Service](https://www.gov.uk/government/statistics/civil-service-statistics-2025/statistical-bulletin-civil-service-statistics-2025#profession) as a whole.
...
Respondents were asked how long that had been in their current role. 
