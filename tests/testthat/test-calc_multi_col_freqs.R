...
test_that("Check the number of columns in output", {
  expect_equal(ncol(dummy_output), 4)
})

test_that("Check the number of rows in output", {
...
test_that("Check the row labels are correctly ordered", {
  expect_equal(dummy_output[[1]], c("column2", "column1", "column3"))
})

test_that("Check the column names in output", {
  expect_equal(colnames(dummy_output), c("labels", "Dummy3", "Dummy2", "Dummy1"))
})
